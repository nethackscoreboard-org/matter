...
$db = NHdb::Db->new(id => 'nhdbfeeder', config => $nhdb);
  my $dbhup = $db->handle();
  my $query = "SELECT map_games_streaks.rowid, map_games_streaks.streaks_i, games.isstreak from map_games_streaks, streaks, games where streaks.num_games > 1 and streaks.streaks_i=map_games_streaks.streaks_i and games.rowid=map_games_streaks.rowid order by games.rowid";
  print $query . "\n";
  my %streakcount = ();
  ...
  for(my $i = 0; $i < scalar(@$re); $i++) {
    my $row = $re->[$i];
    my $rowid = $row->{'rowid'};
    my $streaks_i = $row->{'streaks_i'};
    my $isstreak = $row->{'isstreak'};
    if (!$isstreak) {
	    $isstreak = 0;
    }
	$streakcount{$streaks_i}++;
	if ($streakcount{$streaks_i} > 1) {
		if ($isstreak ne $streakcount{$streaks_i}) {
		my $stmt = "update games set isstreak=? where rowid=?";
		print $stmt . "\t" . $streakcount{$streaks_i} . "\t" . $rowid . "\n";
		$dbhup->do($stmt, undef, $streakcount{$streaks_i}, $rowid);
	}
	}
  }
...
foreach my $variant (@variants) {
  if ($nhdb->first_to_ascend($variant)) {
  my $query = 'SELECT * FROM first_to_ascend(?)';
  print $variant . "\t" . $query . "\n";
  my $re = sql_load($query, 1, 1, $process, $variant);
  for(my $i = 0; $i < scalar(@$re); $i++) {
    my $row = $re->[$i];
    my $rowid = $row->{'rowid'};
    my $isfirst = $row->{'isfirst'};
    if (!$isfirst) {
    	my $stmt = "update games set isfirst='t' where rowid=$rowid";
	print $stmt . "\n";
    	$dbhup->do($stmt);
}
	}
