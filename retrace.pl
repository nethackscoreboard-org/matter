...
sub sql_streak_create_new
{
  ...
  my $r = $sth->execute($logfiles_i, $name_orig, $name_orig);
  if(!$r) {
    $logger->fatal(
      sprintf(
  ...
  $logger->debug(
    sprintf(
      ...
      $streaks_i, $logfiles_i, $name_orig, $rowid
    )
  );
...
sub sql_streak_append_game
{
  ...
  $qry = "SELECT num_games FROM streaks where streaks_i=?";
  my $sth = $dbh->prepare($qry);
  $r = $sth->execute($streaks_i);
  if(!$r) {
    $logger->fatal(
      sprintf(
  ...
  my $result = $sth->fetchrow_hashref();
	if ($result->{'num_games'} > 1) {
		my $stmt = "update games set isstreak=? where rowid=?";
  my $r = $dbh->do($stmt, undef, $result->{'num_games'}, $rowid);
  if(!$r) {
    $logger->fatal(
      sprintf(
        'sql_streak_append_game(%d, %d) failed to set isstreak in game record, errdb=%s',
        $streaks_i, $rowid, $dbh->errstr()
      )
    );
    return $dbh->errstr();
...
sub sql_streak_find
{
  ...
  my $sth = $dbh->prepare($qry);
  my $r = $sth->execute($logfiles_i, $name_orig);
  if(!$r) {
    $logger->fatal(
      sprintf(
        'sql_streak_find(%d, %s) failed, errdb=%s',
        $logfiles_i, $name_orig, $dbh->errstr()
      )
    );
    return $sth->errstr();
  }
  ...
  $logger->debug(
    sprintf(
      ...
      $streaks_i, $logfiles_i, $name_orig
    )
  ) if $streaks_i;
  ...
  #--- perform removals
    ...
    for my $row (@plan_remove) {
      ...
      if($r) {
        ...
        if($r) {
					#this seems wrong if we are tracking steaks independently by alt.
					#    $s = $dbh->do(
...
$sth = $dbh->prepare('SELECT count(*) FROM update');
$r = $sth->execute();
if(!$r) {
...
for my $log (@logfiles) {
  ...
  eval { # <--- eval starts here -------------------------------------------
    ...
    # (table "streaks")
        ...
        if(!exists($streak_open{$logfiles_i}{$parsed_line->{'name_orig'}})) {
          $r = sql_streak_find($logfiles_i, $parsed_line->{'name_orig'});
          die $r if !ref($r);
          $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}} = $r->[0];
        }
    ...
    #--- game is ASCENDED / streak is NOT OPEN
          ...
          if(!$streak_open{$logfiles_i}{$parsed_line->{'name_orig'}}) {
            ...
            $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}} = $streaks_i->[0]
          }
    ...
    # March 19, 2018.
            ...
            my $last_game = sql_streak_get_tail(
              $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}}
            );
            if(!ref($last_game)) {
              die sprintf(
                'sql_streak_get_tail(%s) failed with msg "%s"',
                $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}}, $last_game
              );
            }
            if(
              ...
              $logger->info($lbl,
                sprintf('Closing overlapping streak %d', $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}})
              );
              $r = sql_streak_close(
                $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}}
              );
    ...
    #--- game is not ASCENDED / streak is OPEN
          ...
          if($streak_open{$logfiles_i}{$parsed_line->{'name_orig'}}) {
            ...
            die $r if !ref($r);
            $streak_open{$logfiles_i}{$parsed_line->{'name_orig'}} = undef;
          }
